...
func (state *ThingBreakoutGameGo) resetBall() {
	var old_bx_13 int16 = state.bx
	state.bx = state.padx - state.br / state.SCALE
	var new_bx_13 int16 = state.bx
	var bx_log_13 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0A, byte((((old_bx_13 >> 8) & 0xFF))), byte((((old_bx_13 >> 0) & 0xFF))), byte((((new_bx_13 >> 8) & 0xFF))), byte((((new_bx_13 >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: bx_log_13,
		Size: 7,
	})
	var old_by_3 int16 = state.by
	state.by = state.pady - state.br / state.SCALE
	var new_by_3 int16 = state.by
	var by_log_3 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0B, byte((((old_by_3 >> 8) & 0xFF))), byte((((old_by_3 >> 0) & 0xFF))), byte((((new_by_3 >> 8) & 0xFF))), byte((((new_by_3 >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: by_log_3,
		Size: 7,
	})
	var old_dx_7 int16 = state.dx
	state.dx = (state.padx + state.prevBX + state.prevBY) % 300 - 150
	var new_dx_7 int16 = state.dx
	var dx_log_7 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0C, byte((((old_dx_7 >> 8) & 0xFF))), byte((((old_dx_7 >> 0) & 0xFF))), byte((((new_dx_7 >> 8) & 0xFF))), byte((((new_dx_7 >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: dx_log_7,
		Size: 7,
	})
	if state.dy > 0 {
		var old_dy_27 int16 = state.dy
		state.dy =  -(state.dy)
		var new_dy_27 int16 = state.dy
		var dy_log_27 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_27 >> 8) & 0xFF))), byte((((old_dy_27 >> 0) & 0xFF))), byte((((new_dy_27 >> 8) & 0xFF))), byte((((new_dy_27 >> 0) & 0xFF))) }
		state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
			Payload: dy_log_27,
			Size: 7,
		})
	}
	var old_prevBX_24 int16 = state.prevBX
	state.prevBX =  -(1)
	var new_prevBX_24 int16 = state.prevBX
	var prevBX_log_24 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x11, byte((((old_prevBX_24 >> 8) & 0xFF))), byte((((old_prevBX_24 >> 0) & 0xFF))), byte((((new_prevBX_24 >> 8) & 0xFF))), byte((((new_prevBX_24 >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevBX_log_24,
		Size: 7,
	})
...
func (state *ThingBreakoutGameGo) drawBall() {
	...
	var old_prevBY_20 int16 = state.prevBY
	state.prevBY = (state.by - state.br) / state.SCALE
	var new_prevBY_20 int16 = state.prevBY
	var prevBY_log_20 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x12, byte((((old_prevBY_20 >> 8) & 0xFF))), byte((((old_prevBY_20 >> 0) & 0xFF))), byte((((new_prevBY_20 >> 8) & 0xFF))), byte((((new_prevBY_20 >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevBY_log_20,
		Size: 7,
	})
	var r_17 uint8 = 183
	var g_18 uint8 = 199
...
func (state *ThingBreakoutGameGo) drawPad() {
	var ps int16 = state.padlen / state.SCALE
	state.erasePad()
	var old_prevPX_12 int16 = state.prevPX
	state.prevPX = (state.padx - (state.padlen / 2)) / state.SCALE
	var new_prevPX_12 int16 = state.prevPX
	var prevPX_log_12 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x16, byte((((old_prevPX_12 >> 8) & 0xFF))), byte((((old_prevPX_12 >> 0) & 0xFF))), byte((((new_prevPX_12 >> 8) & 0xFF))), byte((((new_prevPX_12 >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevPX_log_12,
		Size: 7,
	})
	...
	case FragmentControllerMsgsMsgposition:
		if e := typedMessage; port == BreakoutGameGo_BreakoutGame_controller {
			return true, true, state, func() {
				var positionhandled_log_279 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x03, 0x12, 0x00, 0x00, byte((((e.X >> 8) & 0xFF))), byte((((e.X >> 0) & 0xFF))), byte((((e.Y >> 8) & 0xFF))), byte((((e.Y >> 0) & 0xFF))) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: positionhandled_log_279,
					Size: 10,
				})
				var center int32 = (int32(state.RIGHT) - int32(state.LEFT) - int32(state.padlen))
				center = int32(e.X) * center
				center = center / 200
				var old_padx_2 int16 = state.padx
				state.padx = int16((int32(state.LEFT) + center + (int32(state.RIGHT) - int32(state.LEFT)) / 2))
				var new_padx_2 int16 = state.padx
				var padx_log_2 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0E, byte((((old_padx_2 >> 8) & 0xFF))), byte((((old_padx_2 >> 0) & 0xFF))), byte((((new_padx_2 >> 8) & 0xFF))), byte((((new_padx_2 >> 0) & 0xFF))) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: padx_log_2,
					Size: 7,
				})
	...
	case FragmentDisplayMsgsMsgdisplayReady:
		if port == BreakoutGameGo_BreakoutGame_display {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLAUNCH, func() {
				var displayReadyhandled_log_282 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x02, 0x10, 0x01, 0x02 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: displayReadyhandled_log_282,
					Size: 6,
				})
...
func (state *FragmentBreakoutGameStateChartSCStateLAUNCH) OnEntry() {
	...
	var old_countdown_22 uint8 = state.countdown
	state.countdown = 3
	var new_countdown_22 uint8 = state.countdown
	var countdown_log_22 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1E, byte(old_countdown_22), byte(new_countdown_22) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: countdown_log_22,
		Size: 5,
	})
	state.drawScore()
	state.drawLives()
...
func (state *FragmentBreakoutGameStateChartSCStateLAUNCH) Handle(port gosm.Port, message interface{}) (handled bool, internal bool, next gosm.State, action func()) {
	...
	case FragmentTimerMsgsMsgtimer_timeout:
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0 && state.countdown > 0) {
			return true, true, state, func() {
				var timer_timeouthandled_log_275 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x01, 0x16, 0x02, 0x00, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: timer_timeouthandled_log_275,
					Size: 7,
				})
				var id_198 uint8 = 0
				...
				var old_countdown_23 uint8 = state.countdown
				state.countdown = state.countdown - 1
				var new_countdown_23 uint8 = state.countdown
				var countdown_log_23 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1E, byte(old_countdown_23), byte(new_countdown_23) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: countdown_log_23,
					Size: 5,
				})
				state.Send(BreakoutGameGo_BreakoutGame_display, FragmentDisplayMsgsMsgupdate{})
		...
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0 && state.countdown == 0) {
			return true, false, state.FragmentBreakoutGameStateChartSCStatePLAY, func() {
				var timer_timeouthandled_log_274 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x01, 0x16, 0x02, 0x03, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: timer_timeouthandled_log_274,
					Size: 7,
				})
...
func (state *FragmentBreakoutGameStateChartSCStatePLAY) Handle(port gosm.Port, message interface{}) (handled bool, internal bool, next gosm.State, action func()) {
	...
	case FragmentTimerMsgsMsgtimer_timeout:
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0) {
			return true, true, state, func() {
				var timer_timeouthandled_log_276 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x01, 0x16, 0x03, 0x00, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: timer_timeouthandled_log_276,
					Size: 7,
				})
				var old_bx_14 int16 = state.bx
				state.bx = state.bx + state.dx
				var new_bx_14 int16 = state.bx
				var bx_log_14 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0A, byte((((old_bx_14 >> 8) & 0xFF))), byte((((old_bx_14 >> 0) & 0xFF))), byte((((new_bx_14 >> 8) & 0xFF))), byte((((new_bx_14 >> 0) & 0xFF))) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: bx_log_14,
					Size: 7,
				})
				var old_by_4 int16 = state.by
				state.by = state.by + state.dy
				var new_by_4 int16 = state.by
				var by_log_4 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0B, byte((((old_by_4 >> 8) & 0xFF))), byte((((old_by_4 >> 0) & 0xFF))), byte((((new_by_4 >> 8) & 0xFF))), byte((((new_by_4 >> 0) & 0xFF))) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: by_log_4,
					Size: 7,
				})
				...
				if state.bx < wl {
					var old_dx_8 int16 = state.dx
					state.dx =  -(state.dx)
					var new_dx_8 int16 = state.dx
					var dx_log_8 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0C, byte((((old_dx_8 >> 8) & 0xFF))), byte((((old_dx_8 >> 0) & 0xFF))), byte((((new_dx_8 >> 8) & 0xFF))), byte((((new_dx_8 >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: dx_log_8,
						Size: 7,
					})
					var old_bx_15 int16 = state.bx
					state.bx = 2 * wl - state.bx
					var new_bx_15 int16 = state.bx
					var bx_log_15 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0A, byte((((old_bx_15 >> 8) & 0xFF))), byte((((old_bx_15 >> 0) & 0xFF))), byte((((new_bx_15 >> 8) & 0xFF))), byte((((new_bx_15 >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: bx_log_15,
						Size: 7,
					})
					...
					if state.bx > wr {
						var old_dx_9 int16 = state.dx
						state.dx =  -(state.dx)
						var new_dx_9 int16 = state.dx
						var dx_log_9 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0C, byte((((old_dx_9 >> 8) & 0xFF))), byte((((old_dx_9 >> 0) & 0xFF))), byte((((new_dx_9 >> 8) & 0xFF))), byte((((new_dx_9 >> 0) & 0xFF))) }
						state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
							Payload: dx_log_9,
							Size: 7,
						})
						var old_bx_16 int16 = state.bx
						state.bx = 2 * wr - state.bx
						var new_bx_16 int16 = state.bx
						var bx_log_16 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0A, byte((((old_bx_16 >> 8) & 0xFF))), byte((((old_bx_16 >> 0) & 0xFF))), byte((((new_bx_16 >> 8) & 0xFF))), byte((((new_bx_16 >> 0) & 0xFF))) }
						state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
							Payload: bx_log_16,
							Size: 7,
						})
				...
				if state.by < wt {
					var old_dy_28 int16 = state.dy
					state.dy =  -(state.dy)
					var new_dy_28 int16 = state.dy
					var dy_log_28 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_28 >> 8) & 0xFF))), byte((((old_dy_28 >> 0) & 0xFF))), byte((((new_dy_28 >> 8) & 0xFF))), byte((((new_dy_28 >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: dy_log_28,
						Size: 7,
					})
					var old_by_5 int16 = state.by
					state.by = 2 * wt - state.by
					var new_by_5 int16 = state.by
					var by_log_5 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0B, byte((((old_by_5 >> 8) & 0xFF))), byte((((old_by_5 >> 0) & 0xFF))), byte((((new_by_5 >> 8) & 0xFF))), byte((((new_by_5 >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: by_log_5,
						Size: 7,
					})
				...
				if state.dy > 0 {
					if state.by > state.pady - state.br && state.by < state.pady + state.br {
						if state.bx > state.padx - state.padlen / 2 && state.bx < state.padx + state.padlen / 2 {
							var old_dy_29 int16 = state.dy
							state.dy =  -(state.dy)
							var new_dy_29 int16 = state.dy
							var dy_log_29 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_29 >> 8) & 0xFF))), byte((((old_dy_29 >> 0) & 0xFF))), byte((((new_dy_29 >> 8) & 0xFF))), byte((((new_dy_29 >> 0) & 0xFF))) }
							state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
								Payload: dy_log_29,
								Size: 7,
							})
							var old_by_6 int16 = state.by
							state.by = 2 * (state.pady - state.br) - state.by
							var new_by_6 int16 = state.by
							var by_log_6 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0B, byte((((old_by_6 >> 8) & 0xFF))), byte((((old_by_6 >> 0) & 0xFF))), byte((((new_by_6 >> 8) & 0xFF))), byte((((new_by_6 >> 0) & 0xFF))) }
							state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
								Payload: by_log_6,
								Size: 7,
							})
							var old_dx_10 int16 = state.dx
							state.dx = state.dx / 4 + (state.bx - state.padx) / 4
							var new_dx_10 int16 = state.dx
							var dx_log_10 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0C, byte((((old_dx_10 >> 8) & 0xFF))), byte((((old_dx_10 >> 0) & 0xFF))), byte((((new_dx_10 >> 8) & 0xFF))), byte((((new_dx_10 >> 0) & 0xFF))) }
							state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
								Payload: dx_log_10,
								Size: 7,
							})
				...
				if collision {
					var old_dy_30 int16 = state.dy
					state.dy =  -(state.dy)
					var new_dy_30 int16 = state.dy
					var dy_log_30 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_30 >> 8) & 0xFF))), byte((((old_dy_30 >> 0) & 0xFF))), byte((((new_dy_30 >> 8) & 0xFF))), byte((((new_dy_30 >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: dy_log_30,
						Size: 7,
					})
	...
	case FragmentBreakoutGameMsglostBall:
		if port == BreakoutGameGo_BreakoutGame_game {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLOSTBALL, func() {
				var lostBallhandled_log_281 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x05, 0x01, 0x03, 0x04 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: lostBallhandled_log_281,
					Size: 6,
				})
				var id_219 uint8 = 0
		...
		if port == BreakoutGameGo_BreakoutGame_pro_game {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLOSTBALL, func() {
				var lostBallhandled_log_278 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x07, 0x01, 0x03, 0x04 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: lostBallhandled_log_278,
					Size: 6,
				})
				var id_223 uint8 = 0
				state.Send(BreakoutGameGo_BreakoutGame_clock, FragmentTimerMsgsMsgtimer_cancel{
	...
	case FragmentBreakoutGameMsgnextLevel:
		if port == BreakoutGameGo_BreakoutGame_game {
			return true, false, state.FragmentBreakoutGameStateChartSCStateNEXTLEVEL, func() {
				var nextLevelhandled_log_280 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x05, 0x02, 0x03, 0x05 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: nextLevelhandled_log_280,
					Size: 6,
				})
				var id_221 uint8 = 0
				state.Send(BreakoutGameGo_BreakoutGame_clock, FragmentTimerMsgsMsgtimer_cancel{
		...
		if port == BreakoutGameGo_BreakoutGame_pro_game {
			return true, false, state.FragmentBreakoutGameStateChartSCStateNEXTLEVEL, func() {
				var nextLevelhandled_log_277 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x07, 0x02, 0x03, 0x05 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: nextLevelhandled_log_277,
					Size: 6,
				})
				var id_225 uint8 = 0
				state.Send(BreakoutGameGo_BreakoutGame_clock, FragmentTimerMsgsMsgtimer_cancel{
...
func (state *FragmentBreakoutGameStateChartSCStateLOSTBALL) OnEntry() {
	...
	var old_lives_17 uint8 = state.lives
	state.lives = state.lives - 1
	var new_lives_17 uint8 = state.lives
	var lives_log_17 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1C, byte(old_lives_17), byte(new_lives_17) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: lives_log_17,
		Size: 5,
	})
	state.eraseBall()
	state.erasePad()
...
func (state *FragmentBreakoutGameStateChartSCStateLOSTBALL) Handle(port gosm.Port, message interface{}) (handled bool, internal bool, next gosm.State, action func()) {
	...
	case FragmentTimerMsgsMsgtimer_timeout:
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0 && state.lives > 0) {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLAUNCH, func() {
				var timer_timeouthandled_log_271 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x01, 0x16, 0x04, 0x02, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: timer_timeouthandled_log_271,
					Size: 7,
				})
			}
		}
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0 && state.lives == 0) {
			return true, false, state.FragmentBreakoutGameStateChartSCStateGAMEOVER, func() {
				var timer_timeouthandled_log_272 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x01, 0x16, 0x04, 0x06, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: timer_timeouthandled_log_272,
					Size: 7,
				})
			}
		}
...
func (state *FragmentBreakoutGameStateChartSCStateNEXTLEVEL) OnEntry() {
	...
	var old_level_11 uint8 = state.level
	state.level = state.level + 1
	var new_level_11 uint8 = state.level
	var level_log_11 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1D, byte(old_level_11), byte(new_level_11) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: level_log_11,
		Size: 5,
	})
	state.drawLevel()
	state.eraseBall()
	...
	if (state.level % 2) == 0 && state.padlen > 5 * state.SCALE {
		var old_padlen_21 int16 = state.padlen
		state.padlen = state.padlen - (4 * state.SCALE)
		var new_padlen_21 int16 = state.padlen
		var padlen_log_21 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x10, byte((((old_padlen_21 >> 8) & 0xFF))), byte((((old_padlen_21 >> 0) & 0xFF))), byte((((new_padlen_21 >> 8) & 0xFF))), byte((((new_padlen_21 >> 0) & 0xFF))) }
		state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
			Payload: padlen_log_21,
			Size: 7,
		})
	}
	if (state.level % 2) == 1 {
		var old_dy_31 int16 = state.dy
		state.dy = (state.dy * 3) / 2
		var new_dy_31 int16 = state.dy
		var dy_log_31 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_31 >> 8) & 0xFF))), byte((((old_dy_31 >> 0) & 0xFF))), byte((((new_dy_31 >> 8) & 0xFF))), byte((((new_dy_31 >> 0) & 0xFF))) }
		state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
			Payload: dy_log_31,
			Size: 7,
		})
...
func (state *FragmentBreakoutGameStateChartSCStateNEXTLEVEL) Handle(port gosm.Port, message interface{}) (handled bool, internal bool, next gosm.State, action func()) {
	...
	case FragmentTimerMsgsMsgtimer_timeout:
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0) {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLAUNCH, func() {
				var timer_timeouthandled_log_273 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x01, 0x16, 0x05, 0x02, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: timer_timeouthandled_log_273,
					Size: 7,
				})
			}
		}
