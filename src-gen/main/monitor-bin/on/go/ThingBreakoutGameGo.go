...
func (state *ThingBreakoutGameGo) resetBall() {
	var old_bx_0 int16 = state.bx
	state.bx = state.padx - state.br / state.SCALE
	var bx_log_0 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0A, byte((((old_bx_0 >> 8) & 0xFF))), byte((((old_bx_0 >> 0) & 0xFF))), byte((((state.bx >> 8) & 0xFF))), byte((((state.bx >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: bx_log_0,
		Size: 7,
	})
	var old_by_25 int16 = state.by
	state.by = state.pady - state.br / state.SCALE
	var by_log_25 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0B, byte((((old_by_25 >> 8) & 0xFF))), byte((((old_by_25 >> 0) & 0xFF))), byte((((state.by >> 8) & 0xFF))), byte((((state.by >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: by_log_25,
		Size: 7,
	})
	var old_dx_11 int16 = state.dx
	state.dx = (state.padx + state.prevBX + state.prevBY) % 300 - 150
	var dx_log_11 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0C, byte((((old_dx_11 >> 8) & 0xFF))), byte((((old_dx_11 >> 0) & 0xFF))), byte((((state.dx >> 8) & 0xFF))), byte((((state.dx >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: dx_log_11,
		Size: 7,
	})
	if state.dy > 0 {
		var old_dy_20 int16 = state.dy
		state.dy =  -(state.dy)
		var dy_log_20 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_20 >> 8) & 0xFF))), byte((((old_dy_20 >> 0) & 0xFF))), byte((((state.dy >> 8) & 0xFF))), byte((((state.dy >> 0) & 0xFF))) }
		state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
			Payload: dy_log_20,
			Size: 7,
		})
	}
	var old_prevBX_29 int16 = state.prevBX
	state.prevBX =  -(1)
	var prevBX_log_29 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x11, byte((((old_prevBX_29 >> 8) & 0xFF))), byte((((old_prevBX_29 >> 0) & 0xFF))), byte((((state.prevBX >> 8) & 0xFF))), byte((((state.prevBX >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevBX_log_29,
		Size: 7,
	})
	var old_prevBY_15 int16 = state.prevBY
	state.prevBY =  -(1)
	var prevBY_log_15 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x12, byte((((old_prevBY_15 >> 8) & 0xFF))), byte((((old_prevBY_15 >> 0) & 0xFF))), byte((((state.prevBY >> 8) & 0xFF))), byte((((state.prevBY >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevBY_log_15,
		Size: 7,
	})
...
func (state *ThingBreakoutGameGo) eraseBall() {
	...
	var old_prevBX_30 int16 = state.prevBX
	state.prevBX =  -(1)
	var prevBX_log_30 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x11, byte((((old_prevBX_30 >> 8) & 0xFF))), byte((((old_prevBX_30 >> 0) & 0xFF))), byte((((state.prevBX >> 8) & 0xFF))), byte((((state.prevBX >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevBX_log_30,
		Size: 7,
	})
	var old_prevBY_16 int16 = state.prevBY
	state.prevBY =  -(1)
	var prevBY_log_16 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x12, byte((((old_prevBY_16 >> 8) & 0xFF))), byte((((old_prevBY_16 >> 0) & 0xFF))), byte((((state.prevBY >> 8) & 0xFF))), byte((((state.prevBY >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevBY_log_16,
		Size: 7,
	})
...
func (state *ThingBreakoutGameGo) drawBall() {
	var bs int16 = (state.br * 2) / state.SCALE
	state.eraseBall()
	var old_prevBX_31 int16 = state.prevBX
	state.prevBX = (state.bx - state.br) / state.SCALE
	var prevBX_log_31 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x11, byte((((old_prevBX_31 >> 8) & 0xFF))), byte((((old_prevBX_31 >> 0) & 0xFF))), byte((((state.prevBX >> 8) & 0xFF))), byte((((state.prevBX >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevBX_log_31,
		Size: 7,
	})
...
func (state *ThingBreakoutGameGo) drawPad() {
	var ps int16 = state.padlen / state.SCALE
	state.erasePad()
	var old_prevPX_32 int16 = state.prevPX
	state.prevPX = (state.padx - (state.padlen / 2)) / state.SCALE
	var prevPX_log_32 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x16, byte((((old_prevPX_32 >> 8) & 0xFF))), byte((((old_prevPX_32 >> 0) & 0xFF))), byte((((state.prevPX >> 8) & 0xFF))), byte((((state.prevPX >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevPX_log_32,
		Size: 7,
	})
	var old_prevPY_18 int16 = state.prevPY
	state.prevPY = state.pady / state.SCALE
	var prevPY_log_18 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x17, byte((((old_prevPY_18 >> 8) & 0xFF))), byte((((old_prevPY_18 >> 0) & 0xFF))), byte((((state.prevPY >> 8) & 0xFF))), byte((((state.prevPY >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: prevPY_log_18,
		Size: 7,
	})
...
func (state *ThingBreakoutGameGo) incrementScore(diff int8) {
	var old_score_9 int16 = state.score
	state.score = state.score + int16(diff)
	var score_log_9 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1B, byte((((old_score_9 >> 8) & 0xFF))), byte((((old_score_9 >> 0) & 0xFF))), byte((((state.score >> 8) & 0xFF))), byte((((state.score >> 0) & 0xFF))) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: score_log_9,
		Size: 7,
	})
	...
	case FragmentControllerMsgsMsgposition:
		if e := typedMessage; port == BreakoutGameGo_BreakoutGame_controller {
			return true, true, state, func() {
				var positionhandled_log_272 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x00, byte((((e.X >> 8) & 0xFF))), byte((((e.X >> 0) & 0xFF))), byte((((e.Y >> 8) & 0xFF))), byte((((e.Y >> 0) & 0xFF))) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: positionhandled_log_272,
					Size: 7,
				})
				var center int32 = (int32(state.RIGHT) - int32(state.LEFT) - int32(state.padlen))
				center = int32(e.X) * center
				center = center / 200
				var old_padx_4 int16 = state.padx
				state.padx = int16((int32(state.LEFT) + center + (int32(state.RIGHT) - int32(state.LEFT)) / 2))
				var padx_log_4 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0E, byte((((old_padx_4 >> 8) & 0xFF))), byte((((old_padx_4 >> 0) & 0xFF))), byte((((state.padx >> 8) & 0xFF))), byte((((state.padx >> 0) & 0xFF))) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: padx_log_4,
					Size: 7,
				})
	...
	case FragmentDisplayMsgsMsgdisplayReady:
		if port == BreakoutGameGo_BreakoutGame_display {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLAUNCH, func() {
				var displayReadyhandled_log_271 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x01 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: displayReadyhandled_log_271,
					Size: 3,
				})
...
func (state *FragmentBreakoutGameStateChartSCStateLAUNCH) OnEntry() {
	...
	var old_countdown_7 uint8 = state.countdown
	state.countdown = 3
	var countdown_log_7 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1E, byte(old_countdown_7), byte(state.countdown) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: countdown_log_7,
		Size: 5,
	})
	state.drawScore()
	state.drawLives()
...
func (state *FragmentBreakoutGameStateChartSCStateLAUNCH) Handle(port gosm.Port, message interface{}) (handled bool, internal bool, next gosm.State, action func()) {
	...
	case FragmentTimerMsgsMsgtimer_timeout:
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0 && state.countdown > 0) {
			return true, true, state, func() {
				var timer_timeouthandled_log_275 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x02, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
				...
				var old_countdown_8 uint8 = state.countdown
				state.countdown = state.countdown - 1
				var countdown_log_8 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1E, byte(old_countdown_8), byte(state.countdown) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: countdown_log_8,
					Size: 5,
				})
				state.Send(BreakoutGameGo_BreakoutGame_display, FragmentDisplayMsgsMsgupdate{})
		...
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0 && state.countdown == 0) {
			return true, false, state.FragmentBreakoutGameStateChartSCStatePLAY, func() {
				var timer_timeouthandled_log_274 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x03, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
...
func (state *FragmentBreakoutGameStateChartSCStatePLAY) Handle(port gosm.Port, message interface{}) (handled bool, internal bool, next gosm.State, action func()) {
	...
	case FragmentTimerMsgsMsgtimer_timeout:
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0) {
			return true, true, state, func() {
				var timer_timeouthandled_log_276 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x04, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: timer_timeouthandled_log_276,
					Size: 4,
				})
				var old_bx_1 int16 = state.bx
				state.bx = state.bx + state.dx
				var bx_log_1 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0A, byte((((old_bx_1 >> 8) & 0xFF))), byte((((old_bx_1 >> 0) & 0xFF))), byte((((state.bx >> 8) & 0xFF))), byte((((state.bx >> 0) & 0xFF))) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: bx_log_1,
					Size: 7,
				})
				var old_by_26 int16 = state.by
				state.by = state.by + state.dy
				var by_log_26 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0B, byte((((old_by_26 >> 8) & 0xFF))), byte((((old_by_26 >> 0) & 0xFF))), byte((((state.by >> 8) & 0xFF))), byte((((state.by >> 0) & 0xFF))) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: by_log_26,
					Size: 7,
				})
				...
				if state.bx < wl {
					...
					state.dx =  -(state.dx)
					var dx_log_12 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0C, byte((((old_dx_12 >> 8) & 0xFF))), byte((((old_dx_12 >> 0) & 0xFF))), byte((((state.dx >> 8) & 0xFF))), byte((((state.dx >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: dx_log_12,
						Size: 7,
					})
					var old_bx_2 int16 = state.bx
					state.bx = 2 * wl - state.bx
					var bx_log_2 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0A, byte((((old_bx_2 >> 8) & 0xFF))), byte((((old_bx_2 >> 0) & 0xFF))), byte((((state.bx >> 8) & 0xFF))), byte((((state.bx >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: bx_log_2,
						Size: 7,
					})
					...
					if state.bx > wr {
						...
						state.dx =  -(state.dx)
						var dx_log_13 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0C, byte((((old_dx_13 >> 8) & 0xFF))), byte((((old_dx_13 >> 0) & 0xFF))), byte((((state.dx >> 8) & 0xFF))), byte((((state.dx >> 0) & 0xFF))) }
						state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
							Payload: dx_log_13,
							Size: 7,
						})
						var old_bx_3 int16 = state.bx
						state.bx = 2 * wr - state.bx
						var bx_log_3 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0A, byte((((old_bx_3 >> 8) & 0xFF))), byte((((old_bx_3 >> 0) & 0xFF))), byte((((state.bx >> 8) & 0xFF))), byte((((state.bx >> 0) & 0xFF))) }
						state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
							Payload: bx_log_3,
							Size: 7,
						})
				...
				if state.by < wt {
					var old_dy_21 int16 = state.dy
					state.dy =  -(state.dy)
					var dy_log_21 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_21 >> 8) & 0xFF))), byte((((old_dy_21 >> 0) & 0xFF))), byte((((state.dy >> 8) & 0xFF))), byte((((state.dy >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: dy_log_21,
						Size: 7,
					})
					var old_by_27 int16 = state.by
					state.by = 2 * wt - state.by
					var by_log_27 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0B, byte((((old_by_27 >> 8) & 0xFF))), byte((((old_by_27 >> 0) & 0xFF))), byte((((state.by >> 8) & 0xFF))), byte((((state.by >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: by_log_27,
						Size: 7,
					})
				...
				if state.dy > 0 {
					if state.by > state.pady - state.br && state.by < state.pady + state.br {
						if state.bx > state.padx - state.padlen / 2 && state.bx < state.padx + state.padlen / 2 {
							var old_dy_22 int16 = state.dy
							state.dy =  -(state.dy)
							var dy_log_22 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_22 >> 8) & 0xFF))), byte((((old_dy_22 >> 0) & 0xFF))), byte((((state.dy >> 8) & 0xFF))), byte((((state.dy >> 0) & 0xFF))) }
							state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
								Payload: dy_log_22,
								Size: 7,
							})
							...
							var dx_log_14 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0C, byte((((old_dx_14 >> 8) & 0xFF))), byte((((old_dx_14 >> 0) & 0xFF))), byte((((state.dx >> 8) & 0xFF))), byte((((state.dx >> 0) & 0xFF))) }
							state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
								Payload: dx_log_14,
								Size: 7,
							})
				...
				if collision {
					var old_dy_23 int16 = state.dy
					state.dy =  -(state.dy)
					var dy_log_23 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_23 >> 8) & 0xFF))), byte((((old_dy_23 >> 0) & 0xFF))), byte((((state.dy >> 8) & 0xFF))), byte((((state.dy >> 0) & 0xFF))) }
					state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
						Payload: dy_log_23,
						Size: 7,
					})
	...
	case FragmentBreakoutGameMsglostBall:
		if port == BreakoutGameGo_BreakoutGame_game {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLOSTBALL, func() {
				var lostBallhandled_log_281 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x05 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		...
		if port == BreakoutGameGo_BreakoutGame_pro_game {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLOSTBALL, func() {
				var lostBallhandled_log_279 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x07 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: lostBallhandled_log_279,
					Size: 3,
				})
				var id_223 uint8 = 0
				state.Send(BreakoutGameGo_BreakoutGame_clock, FragmentTimerMsgsMsgtimer_cancel{
	...
	case FragmentBreakoutGameMsgnextLevel:
		if port == BreakoutGameGo_BreakoutGame_game {
			return true, false, state.FragmentBreakoutGameStateChartSCStateNEXTLEVEL, func() {
				var nextLevelhandled_log_282 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x06 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
					Payload: nextLevelhandled_log_282,
					Size: 3,
				})
				var id_221 uint8 = 0
				state.Send(BreakoutGameGo_BreakoutGame_clock, FragmentTimerMsgsMsgtimer_cancel{
		...
		if port == BreakoutGameGo_BreakoutGame_pro_game {
			return true, false, state.FragmentBreakoutGameStateChartSCStateNEXTLEVEL, func() {
				var nextLevelhandled_log_280 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x08 }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
...
func (state *FragmentBreakoutGameStateChartSCStateLOSTBALL) OnEntry() {
	...
	var old_lives_5 uint8 = state.lives
	state.lives = state.lives - 1
	var lives_log_5 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1C, byte(old_lives_5), byte(state.lives) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: lives_log_5,
		Size: 5,
	})
...
func (state *FragmentBreakoutGameStateChartSCStateLOSTBALL) Handle(port gosm.Port, message interface{}) (handled bool, internal bool, next gosm.State, action func()) {
	...
	case FragmentTimerMsgsMsgtimer_timeout:
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0 && state.lives > 0) {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLAUNCH, func() {
				var timer_timeouthandled_log_277 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x09, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		...
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0 && state.lives == 0) {
			return true, false, state.FragmentBreakoutGameStateChartSCStateGAMEOVER, func() {
				var timer_timeouthandled_log_278 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x0A, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
...
func (state *FragmentBreakoutGameStateChartSCStateNEXTLEVEL) OnEntry() {
	...
	var old_level_6 uint8 = state.level
	state.level = state.level + 1
	var level_log_6 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x1D, byte(old_level_6), byte(state.level) }
	state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
		Payload: level_log_6,
		Size: 5,
	})
	...
	if (state.level % 2) == 0 && state.padlen > 5 * state.SCALE {
		var old_padlen_19 int16 = state.padlen
		state.padlen = state.padlen - (4 * state.SCALE)
		var padlen_log_19 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x10, byte((((old_padlen_19 >> 8) & 0xFF))), byte((((old_padlen_19 >> 0) & 0xFF))), byte((((state.padlen >> 8) & 0xFF))), byte((((state.padlen >> 0) & 0xFF))) }
		state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
			Payload: padlen_log_19,
			Size: 7,
		})
	}
	if (state.level % 2) == 1 {
		var old_dy_24 int16 = state.dy
		state.dy = (state.dy * 3) / 2
		var dy_log_24 []byte = []byte{ LogType_property_changed, state.DEBUG_BIN_ID, 0x0D, byte((((old_dy_24 >> 8) & 0xFF))), byte((((old_dy_24 >> 0) & 0xFF))), byte((((state.dy >> 8) & 0xFF))), byte((((state.dy >> 0) & 0xFF))) }
		state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
			Payload: dy_log_24,
			Size: 7,
		})
...
func (state *FragmentBreakoutGameStateChartSCStateNEXTLEVEL) Handle(port gosm.Port, message interface{}) (handled bool, internal bool, next gosm.State, action func()) {
	...
	case FragmentTimerMsgsMsgtimer_timeout:
		if t := typedMessage; port == BreakoutGameGo_BreakoutGame_clock && (t.Id == 0) {
			return true, false, state.FragmentBreakoutGameStateChartSCStateLAUNCH, func() {
				var timer_timeouthandled_log_273 []byte = []byte{ LogType_message_handled, state.DEBUG_BIN_ID, 0x0B, byte(t.Id) }
				state.Send(BreakoutGameGo_WithBinaryLog_log, FragmentLogMsgsMsglog{
