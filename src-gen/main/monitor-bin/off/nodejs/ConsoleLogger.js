...
ConsoleLogger.prototype.do_log = function(ConsoleLogger_do_log_payload_var, ConsoleLogger_do_log_size_var) {
	if( !(this.ConsoleLogger_QUIET_var)) {
	const log_type_var = ConsoleLogger_do_log_payload_var[0];
	...
	((process.stdout && process.stdout.write) || console.log).call(process.stdout, ''+'log: unknown          [');
	...
	const v_var = ConsoleLogger_do_log_payload_var[i_var];
	if(this.Logger_HAS_SIGNED_BYTE_var) {
	((process.stdout && process.stdout.write) || console.log).call(process.stdout, ''+(v_var & 0xFF));
	
	} else {
	((process.stdout && process.stdout.write) || console.log).call(process.stdout, ''+v_var);
